...
export function BattleTableSquareMine(){
    ...
    const [target, setTarget] = useState(['A1', 'A2', 'A3', 'A4', 'A5', 'C6', 'C7', 'C8', 'C9', 'F7', 'G7', 'H7', 'D1', 'G3', 'G4', 'F8']);
    ...
    const getShipPeace = (key, num) => {
        let res = ships.some(p => p.l.find( l => l === String(key+(num+1))));
        return res;
    }

    const isWhatPeace = (key, num) => {
        let idx = key+(num+1);
    ...
    keys.forEach(({ key }, i) => {
        keys[i].value.map((e, num) => {
            return rows.push(<BattleTableSquareItem key={position(key, num)}>{
                ...
                <BattleTableSquareItemPosition>{position(key, num)}</BattleTableSquareItemPosition>
